...
;(function ( $, window, undefined ) {
    ...
    Plugin.prototype.init = function () {
        ...
        this._dateDebut = new moment(this.$element.find('li:first time').attr('datetime'), 'YYYY');
        this._dateFin = new moment(this.$element.find('li:last time').attr('datetime'), 'YYYY');
        this._duree = this._dateFin.diff(this._dateDebut)-10;
		//this._dateFin._i = 95;
		console.log(this._dateFin);
        // Placement de l'évenement
        this.$element.find('li').each(function () {
            $li = $(this);
            $li.wrapInner('<div class="event" />');
            var date = new moment($li.find('time').attr('datetime'), 'YYYY')
